...
const ManageUsers = () => {
  ...
  const [porcentaje, setPorcentaje] = useState(4); // Por defecto 4
    const [credito, setCredito] = useState(false); // Por defecto 4
  const navigate = useNavigate();

  const navigateToRecargasHechas = (user) => {
    navigate(`/vendedor/usuario/${user}`);
  };
  const fetchUsers = async () => {
    const token = localStorage.getItem('token');
    try {
      const response = await axios.get('https://www.recargacreditos.com.mx/api/admin/obtener-tiendas/usuario', {
        headers: {
    ...
    try {
      const response = await axios.post(
        `https://www.recargacreditos.com.mx/api/admin/acreditar-saldo2/${selectedUser.usuario.correo}`,
        {
          ...
          credito: credito // Enviamos el estado del toggle de crédito
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          {selectedUser ? (
            <div>
              ...
              <Form>
                ...
                <Form.Group>
                  <Form.Label>Tipo de saldo (Crédito)</Form.Label>
                  <ToggleSwitch
                    id="credito-toggle"
                    checked={credito}
                    onChange={() => setCredito(!credito)} // Alternar el valor de crédito
                  />
                </Form.Group>
              </Form>
            </div>

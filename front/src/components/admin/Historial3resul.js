  ...
  const fetchData = async (option) => {
    ...
    try {
      const response = await api.get(
        `/admin/historial4/${option.toLowerCase()}`,
        {
          headers: {
    ...
    const filtered = data.filter((item) => {
      ...
      if (selectedOption.toLowerCase() === "saldos") {
        dateToCheck = item.fecha ? parseISO(item.fecha) : null;
          ...
          dateToCheck = item.fecha ? parseISO(item.fecha) : null;
        } else if (selectedOption.toLowerCase() === "pagos") {
          dateToCheck = item.fecha ? parseISO(item.fecha) : null;
...
{selectedOption.toLowerCase() === "pagos" && (
          <>
            <td>
              {item.fecha ? new Date(item.fecha).toLocaleDateString() : "N/A"}
            </td>
  ...
  const exportToExcel = async () => {
    const optionMap = {
      ...
      Abonos: {
        internalOption: "abonos",
        title: "Abonos",
      },
      Pagos: {
        internalOption: "pagos",
        title: "Pagos",
      },
      Depositos: {
        internalOption: "recepción de saldo",
        title: "Recepción de saldo",
      },
      ...
      <Row className="mb-1 align-items-center">
        <Col xs={12} md={4} className="mb-1">
          ...
          >
            ...
            <Dropdown.Item eventKey="Pagos">Pagos</Dropdown.Item>
